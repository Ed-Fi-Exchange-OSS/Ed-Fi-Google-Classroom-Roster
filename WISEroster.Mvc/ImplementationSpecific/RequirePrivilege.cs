...
namespace WISEroster.Mvc.ImplementationSpecific
{
    public class RequirePrivilege : FilterAttribute, IAuthorizationFilter
    {
        ...
        public void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            var missingPrivileges = _privileges.Where(privilege =>
                user.HasPrivilege(sessionInfo.CurrentAgencyId.GetValueOrDefault(), privilege) == false);
            if (missingPrivileges.Any())
            {
                throw new HttpException((int) HttpStatusCode.Forbidden,
                    string.Format("User is missing required privileges: {0}", string.Join(", ", missingPrivileges)));
            }
